import { n as __commonJSMin, t as require_react } from "./react-DAfEDbDl.js";
...
var require_react_dom_client_development = /* @__PURE__ */ __commonJSMin(((exports) => {
	(function() {
				...
				do {
					...
					if (debugInfo) for (var i = debugInfo.length - 1; 0 <= i; i--) {
						...
						if ("string" === typeof entry.name) {
							...
							info = JSCompiler_temp_const + describeBuiltInComponentFrame(entry.name + (env ? " [" + env + "]" : ""));
						}
					}
		...
		function pushHostContainer(fiber, nextRootInstance) {
			...
			switch (nextRootContext) {
				...
				case 11:
					...
					break;
				default: if (nextRootInstance = 8 === nextRootContext ? nextRootInstance.parentNode : nextRootInstance, nextRootContext = nextRootInstance.tagName, nextRootInstance = nextRootInstance.namespaceURI) nextRootInstance = getOwnHostContext(nextRootInstance), nextRootInstance = getChildHostContextProd(nextRootInstance, nextRootContext);
		...
		function isTagValidWithParent(tag, parentTag) {
			switch (parentTag) {
				case "select": return "hr" === tag || "option" === tag || "optgroup" === tag || "#text" === tag;
				...
				case "html": return "head" === tag || "body" === tag || "frameset" === tag;
				case "frameset": return "frame" === tag;
				case "#document": return "html" === tag;
			}
			switch (tag) {
				case "h1":
				case "h2":
		...
		function validateTextNesting(childText, parentTag) {
			...
			warnKey = null !== warnKey && null !== ancestor ? describeAncestors(ancestor, warnKey, 6 !== warnKey.tag ? { children: null } : null) : "";
			...
			function warnOnInvalidKey(returnFiber, workInProgress, child, knownKeys) {
				...
				switch (child.$$typeof) {
					...
					case REACT_PORTAL_TYPE:
						...
						break;
					case REACT_LAZY_TYPE: child = callLazyInitInDEV(child), warnOnInvalidKey(returnFiber, workInProgress, child, knownKeys);
				}
				return knownKeys;
			}
					...
					if (null !== JSCompiler_object_inline_digest_2317) {
						...
						if (0 !== (JSCompiler_object_inline_stack_2318 & 42)) JSCompiler_object_inline_stack_2318 = 1;
							...
							case 268435456:
								...
								break;
							default: JSCompiler_object_inline_stack_2318 = 0;
						}
		...
		function commitMutationEffectsOnFiber(finishedWork, root) {
			...
			switch (finishedWork.tag) {
				...
				case 26:
					...
					break;
				case 27: if (flags & 4 && null === finishedWork.alternate) {
		...
		function completeWork(current, workInProgress, renderLanes) {
			...
			switch (workInProgress.tag) {
							...
							switch (_type) {
								...
								default: switch (renderLanes) {
									...
									case "script":
										...
										break;
									case "select":
										current = "string" === typeof newProps.is ? current.createElement("select", { is: newProps.is }) : current.createElement("select");
										newProps.multiple ? current.multiple = !0 : newProps.size && (current.size = newProps.size);
										break;
									default: current = "string" === typeof newProps.is ? current.createElement(renderLanes, { is: newProps.is }) : current.createElement(renderLanes), -1 === renderLanes.indexOf("-") && (renderLanes !== renderLanes.toLowerCase() && console.error("<%s /> is using incorrect casing. Use PascalCase for React components, or lowercase for HTML elements.", renderLanes), "[object HTMLUnknownElement]" !== Object.prototype.toString.call(current) || hasOwnProperty.call(warnedUnknownTags, renderLanes) || (warnedUnknownTags[renderLanes] = !0, console.error("The tag <%s> is unrecognized in this browser. If you meant to render a React component, start its name with an uppercase letter.", renderLanes)));
								}
							}
						...
						if (popHydrationState(workInProgress)) {
							...
							if (null !== returnFiber) switch (returnFiber.tag) {
								case 3:
									...
									break;
								case 27:
								case 5: renderLanes = returnFiber.memoizedProps, _type && (_type = diffHydratedTextForDevWarnings(current, newProps, renderLanes), null !== _type && (buildHydrationDiffNode(workInProgress, 0).serverProps = _type));
							}
							current[internalInstanceKey] = workInProgress;
				...
				case 24: return newProps = null, null !== current && (newProps = current.memoizedState.cache), workInProgress.memoizedState.cache !== newProps && (workInProgress.flags |= 2048), popProvider(CacheContext, workInProgress), bubbleProperties(workInProgress), null;
				case 25: return null;
			}
			throw Error("Unknown unit of work tag (" + workInProgress.tag + "). This error is likely caused by a bug in React. Please file an issue.");
		}
		...
		function bindToConsole(methodName, args, badgeName) {
			...
			switch (methodName) {
				...
				case "dirxml":
				case "groupEnd":
				case "table": return bind.apply(console[methodName], [console].concat(args));
				case "assert": offset = 1;
			}
			args = args.slice(0);
		...
		} }, callComponentDidCatchInDEV = callComponentDidCatch["react-stack-bottom-frame"].bind(callComponentDidCatch), callComponentWillUnmount = { "react-stack-bottom-frame": function(current, nearestMountedAncestor, instance) {
			try {
		...
		} }, callCreateInDEV = callCreate["react-stack-bottom-frame"].bind(callCreate), callDestroy = { "react-stack-bottom-frame": function(current, nearestMountedAncestor, destroy) {
			try {
				destroy();
			} catch (error) {
				captureCommitPhaseError(current, nearestMountedAncestor, error);
